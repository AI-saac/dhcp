   ...
   */
    ...
    while (!quit_flag) {
        if (st == STATE_INIT) {
            ...
            send_to(sd, &discover_msg, sizeof discover_msg, 0, (struct sockaddr*)&skt,
                sktlen);
            ...
            if (n == 0) {
                ...
                if (FD_ISSET(sd, &rdfds)) {
                    recv_from(sd, &offer_msg, sizeof offer_msg, 0, (struct sockaddr*)&skt,
                        &sktlen);
            ...
            if (n == 0) {
                ...
                recv_from(sd, &reply_msg, sizeof reply_msg, 0, (struct sockaddr*)&skt,
                    &sktlen);
...
void trap(int no)
{
    if (assigned_ip != -1 && assigned_netmask != -1) {
        ...
        send_to(sd, &release_msg, sizeof release_msg, 0, (struct sockaddr*)&skt,
            sktlen);
    }
    exit(EXIT_SUCCESS);
}
...
void timeout(int no)
{
    ...
    if (st == STATE_NEED_EXT) {
        ...
        send_to(sd, &request_msg, sizeof request_msg, 0, (struct sockaddr*)&skt,
            sktlen);
        set_state(STATE_WAIT_REPLY);
    }
    else {
